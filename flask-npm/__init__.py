...
class NPM(object):
    ...
    def __init__(self, app):
        self.app = app
        self.setdefault(app)
        self.root = Path(app.config['NPM_DIR'])
        self.static_root = Path(app.config['STATIC_DIR'])
        self.app.add_url_rule(
            '/{prefix}/<name>/<path:file>'.format(prefix=app.config['NPM_PREFIX']),
            view_func=self.serve_static)

    @staticmethod
    def setdefault(app):
        app.config.setdefault('NPM_PREFIX', 'static')
        app.config.setdefault('STATIC_DIR', 'static')
        app.config.setdefault('NPM_DIR', 'node_modules')
